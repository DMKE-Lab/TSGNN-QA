...
class MsgLinkPredictor(nn.Module):
    ...
    def __init__(self, num_nodes, h_dim, out_dim):
        ...
        self.src_fc = nn.Linear(h_dim, out_dim)
        self.dst_fc = nn.Linear(h_dim, out_dim)
        self.out_fc = nn.Linear(h_dim, 1)
...
class TimeEncode(nn.Module):
    ...
    def __init__(self, num_nodes, h_dim, out_dim):
        ...
        self.pos_h_e = nn.Embedding(self.h_dim, self.num_nodes)
        self.w = torch.nn.Linear(1, self.num_nodes)
        self.w.weight = torch.nn.Parameter((torch.from_numpy(1 / 10 ** np.linspace(0, 9, self.num_nodes)))
                                           .double().reshape(self.num_nodes, -1))
        self.w.bias = torch.nn.Parameter(torch.zeros(self.num_nodes).double())

    def time_projection(self, data, t):
        """
        this is: Pt(eh)=eh - (wTt * eh) * wt
        """
        torch.sum(data * t, axis=1)
        inner_prod = torch.repeat(torch.unsqueeze(torch.sum(data * t, axis=1), axis=1), [1, self.p.inp_dim])
        prod = t * inner_prod
        data = data - prod
        return data
