...
sys.path.append("..")
from TGN import utils
from TGN.utils import build_sub_graph
from src.hyperparameter_range import hp_range
import torch.nn.modules.rnn
from collections import defaultdict
from TGN.knowledge_graph import _read_triplets_as_list



def id2str_entity(dataset):
    id2str_entity_dict = dict()
    file_path = os.path.join("../data/", dataset)
    file_name = os.path.join(file_path, 'entity2id.txt')
    with open(file_name, 'r+', encoding='UTF-8') as f:
        for line in f.readlines():
            entity_name, entity_index = line.strip().split('\t')[:2]
            id2str_entity_dict[int(entity_index)] = entity_name
    return id2str_entity_dict


def id2str_relation(dataset):
    id2str_relation_dict = dict()
    file_path = os.path.join("../data/", dataset)
    file_name = os.path.join(file_path, 'relation2id.txt')
    with open(file_name, 'r+', encoding='UTF-8') as f:
        for line in f.readlines():
            rel_name, rel_index = line.strip().split('\t')[:2]
            id2str_relation_dict[int(rel_index)] = rel_name
    return id2str_relation_dict


def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name,
         static_graph, global_graph, Formal, mode):
    """
    ...
    if mode == "test":
        ...
        Formal = checkpoint['Formal']
    model.eval()
    # do not have inverse relation in test input
    id2entity = id2str_entity(args.dataset)
    id2relation = id2str_relation(args.dataset)

    input_list = [snap for snap in history_list[-args.test_history_len:]]
    result_maitain = open("text_results.txt", "w+", encoding='UTF-8')
    # total_num = 0

    # fp = open('../data/' + str("ICEWS14s") + '/adv.txt', 'w+')

    for time_idx, test_snap in enumerate(tqdm(test_list)):
        history_glist = [build_sub_graph(num_nodes, num_rels, g, use_cuda, args.gpu) for g in input_list]
        test_triples_input = torch.LongTensor(test_snap).cuda() if use_cuda else torch.LongTensor(test_snap)
        test_triples_input = test_triples_input.to(args.gpu)
        test_triples, final_score, final_r_score = model.predict(history_glist, num_rels, static_graph, global_graph,
                                                                 test_triples_input, use_cuda, Formal)

        softmax = torch.nn.Softmax(1)
        final_softmax_score = softmax(final_score)
        prop, index = final_softmax_score.topk(5, 1)

        # for ind,test_input in enumerate(test_triples_input):
        #    ob_pred = test_input[2]

        #    if ob_pred not in index[ind]:
        #        total_num += 1
        #        fp.write(str(int(test_input[0])) + '\t' + str( int(test_input[1])) + '\t' + str(  int(test_input[2])) + '\t' + str(int(time_idx)//2 + 12) + '\n')

        prop = prop.cpu()
        index = index.cpu()
        for i in range(len(index)):
            if int(test_triples[i, 1]) < len(id2relation):
                result_maitain.write(
                    str(float(prop[i, 0])) + '\t' + str(float(prop[i, 1])) + '\t' + str(float(prop[i, 2])) + '\t' + str(
                        float(prop[i, 3])) + '\t' + str(float(prop[i, 4])) + '\n')
                result_maitain.write(
                    str(id2entity[int(index[i, 0])]) + '\t' + str(id2entity[int(index[i, 1])]) + '\t' + str(
                        id2entity[int(index[i, 2])]) + '\t' + str(id2entity[int(index[i, 3])]) + '\t' + str(
                        id2entity[int(index[i, 4])]) + '\n')
                result_maitain.write(str(id2entity[int(test_triples[i, 0])]) + '\t' + str(
                    id2relation[int(test_triples[i, 1])]) + '\t' + str(id2entity[int(test_triples[i, 2])]) + '\t' + str(
                    time_idx) + '\n')
...
def get_init_model(args, num_nodes, num_rels, num_static_rels, num_words, use_cuda):
    from TGN.tgn import TGN
    model = TGN(args.decoder,
                          args.encoder,
                          num_nodes,
                          num_rels,
                          num_static_rels,
                          ...
                          use_global=args.add_global_graph,
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    if args.add_entity_dissolution:
        data = utils.load_data_adv(args.dataset)
        adv_list = utils.split_by_time(data.adv_train)
    else:
    ...
    model_name = "{}-{}-{}-ly{}-dilate{}-his{}-weight{}-discount{}-angle{}-dp{}{}{}{}" \
    ...
    global_triplets = utils.split_by_global(data.train)  #
    if args.add_global_graph:
        global_graph = build_sub_graph(num_nodes, num_rels, global_triplets, use_cuda, args.gpu)
    else:
        global_graph = None
    # create stat
    model = get_init_model(args, num_nodes, num_rels, num_static_rels, num_words, use_cuda)
    model2 = get_init_model(args, num_nodes, num_rels, num_static_rels, num_words, use_cuda)
    ...
    if args.test and os.path.exists(model_state_file):
        mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r = test(model2,
        ...
        for train_sample_num in tqdm(idx):
            ...
            Formal = [False,False]
            loss_e, loss_r, loss_static,loss_parm,time_weight = model.get_loss(history_glist, output[0],None, static_graph,sub_global_graph,use_cuda, Formal)
            loss = args.task_weight*loss_e + (1-args.task_weight)*loss_r + 1* loss_parm + loss_static
            loss.backward()
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                if args.add_entity_dissolution:
                    if len(np.array(adv_list[train_sample_num:train_sample_num + 1]).shape) == 3:
                        adv = adv_list[train_sample_num:train_sample_num + 1]
                if train_sample_num - args.train_history_len < 0:
                    input_list = train_list[0: train_sample_num]
                else:
                ...
                if args.add_entity_dissolution:
                    if adv is not None:
                        adv = [torch.from_numpy(_).long().cuda() for _ in adv] if use_cuda else [
                            torch.from_numpy(_).long() for _ in adv]
                        adv = adv[0]
                if time_weight < 0.5:  # transient
                    Formal = [True, False]
                else:
                    Formal = [True, True]
                loss_e, loss_r, loss_static, loss_parm, _ = model.get_loss(history_glist, output[0], adv, static_graph,
                                                                           global_graph, use_cuda, Formal)
                loss = args.task_weight * loss_e + (1 - args.task_weight) * loss_r + 1 * loss_static
            ...
            if epoch and epoch % args.evaluate_every == 0:
                ...
                if not args.relation_evaluation:  # entity prediction evalution
                    if mrr_filter < best_mrr:
                        ...
                        best_mrr = mrr_filter
                        torch.save({'state_dict': model.state_dict(), 'epoch': epoch, 'Formal': Formal},
                                   model_state_file)
                else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='TGN')
    ...
    # configuration for encoder TGN stat
    ...
    parser.add_argument("--skip-connect", action='store_true', default=False,
                        help="whether to use skip connect in a tgn Unit")
    ...
    parser.add_argument("--n-basis", type=int, default=100,
                        help="number of basis vector for tgn")
    parser.add_argument("--n-layers", type=int, default=2,
                        help="number of propagation rounds")
    parser.add_argument("--self-loop", action='store_true', default=True,
                        help="perform layer normalization in every layer of tgn ")
    ...
    parser.add_argument("--add-global-graph", action='store_true', default=False,
                        help="use the info of global graph")
    parser.add_argument("--add-entity-dissolution", action='store_true', default=False,
                        help="use the info of entity-dissolution")
