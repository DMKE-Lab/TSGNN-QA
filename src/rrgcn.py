...
from TGN.layers import ResUnionRGCNLayer, UnionRGCNLayer, RGCNBlockLayer
...
class RecurrentRGCN(nn.Module):
    ...
    def forward(self, g_list, static_graph, global_graph, use_cuda, Formal):
        ...
        batched_g = self.rgcn.forward(batched_g, self.gruh, [self.init_r, self.init_r])
        batched_glist = dgl.unbatch(batched_g, batch_num_nodes, batch_num_edges)

        for i, current_g in enumerate(batched_glist):
            # for i, current_g in enumerate(g_list):
            his_h = None
            """
            current_g = current_g.to(self.gpu)
            current_g = self.rgcn.forward(current_g, self.gruh, [self.init_r, self.init_r])
            """
            current_h = F.normalize(current_g.ndata['h']) if self.layer_norm else current_g.ndata['h']

            self.h = F.normalize(self.h_cell_1(current_h, self.h)) if self.layer_norm else self.h_cell_1(current_h,
                                                                                                         self.h)
            ...
            if i == len(g_list) - 1:
                ...
                his_g = self.rgcn2.forward(transient_g.to(self.gpu), self.init_h, [self.init_r, self.init_r])
                his_h = F.normalize(his_g.ndata['h']) if self.layer_norm else his_g.ndata['h']
                ...
                if not train_flag:
                    ...
                    if memory_flag:
                        time_weight = torch.ones(self.time_gate_bias.shape).to(self.gpu) if use_cuda else torch.zeros(
                            self.time_gate_bias.shape)
                    else:
                        time_weight = torch.zeros(self.time_gate_bias.shape).to(self.gpu) if use_cuda else torch.ones(
                            self.time_gate_bias.shape)
                    his_h = time_weight * his_h + (1 - time_weight) * self.h
        ...
        return history_embs, calculate_embs, static_emb, self.init_r, torch.mean(time_weight)
    ...
    def predict(self, test_graph, num_rels, static_graph, global_graph, test_triplets, use_cuda, Formal):
        with torch.no_grad():
            ...
            evolve_embs, calculate_embs, static_emb, r_emb, _ = self.forward(test_graph, static_graph, global_graph,
                                                                             use_cuda, Formal)
    ...
    def get_loss(self, glist, triples, adv_triples, static_graph, global_graph, use_cuda, Formal):
        ...
        evolve_embs, calculate_embs, static_emb, r_emb, time_weight = self.forward(glist, static_graph, global_graph,
                                                                                   use_cuda, Formal)
        ...
        if self.entity_prediction:
            ...
            if adv_triples is not None:
                ...
                loss_ent -= 0.01 * torch.clip(self.loss_e(scores_adv_ob, all_adv_triples[:, 2]), 0, 3)
            ...
            if self.discount == 1:
                for time_step, evolve_emb in enumerate(calculate_embs):
                    ...
                    if self.layer_norm:
                        ...
                        c = torch.norm(static_emb, p=2, dim=1) * torch.norm(evolve_emb[:, :static_emb.shape[1]], p=2,
                                                                            dim=1)
                        sim_matrix = sim_matrix / c
                    mask = (math.cos(step) - sim_matrix) > 0
                    # loss_static += 0.1*(10-time_step) * torch.mean(- sim_matrix)
